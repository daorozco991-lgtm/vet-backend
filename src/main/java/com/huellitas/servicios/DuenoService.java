...
public class DuenoService {
    ...
    public DuenoDto actualizar(Integer mascotaId, DuenoPatchDto duenoPatchDto) {
        Dueno duenoContacto = duenoRepository.findByContacto(duenoPatchDto.getContacto());


        Dueno dueno = duenoRepository.findById(mascotaId).orElseThrow(() -> new NegocioException("No existe un dueño con id: " + mascotaId));
        if (!Objects.equals(duenoContacto.getId(), dueno.getId())) {
            throw new NegocioException("Ya hay un dueño registrado con ese número de telefono");
        }
